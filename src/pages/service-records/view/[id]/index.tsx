...
import { getServiceRecordById } from 'apiSdk/service-records';
import { ServiceRecordInterface } from 'interfaces/service-record';

function ServiceRecordViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<ServiceRecordInterface>(
    () => (id ? `/service-records/${id}` : null),
    () =>
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('service_record', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/service-records/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                {hasAccess('branch', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Branch"
                    text={
                      <Link as={NextLink} href={`/branches/view/${data?.branch?.id}`}>
                        {data?.branch?.name}
                      </Link>
                    }
                  />
