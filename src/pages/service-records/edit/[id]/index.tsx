...
import { getServiceRecordById, updateServiceRecordById } from 'apiSdk/service-records';
...
function ServiceRecordEditPage() {
  ...
  const handleSubmit = async (values: ServiceRecordInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateServiceRecordById(id, values);
      mutate(updated);
      resetForm();
